...
export function PackageForm({
  ...
  readonly initialValues: PackageFormValues;
  readonly isSubmitting: boolean;
  readonly submitLabel: string;
  readonly includeStatus: boolean;
  readonly onSubmit: (values: PackageFormValues) => void;
}) {
  ...
  function updateValue<TKey extends keyof PackageFormValues>(key: TKey, value: PackageFormValues[TKey]) {
    ...
    <form className="grid gap-5" onSubmit={(event) => { event.preventDefault(); onSubmit(values); }}>
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {includeStatus ? (
          <label className="grid gap-2 text-[14px] font-medium">
            Status
            <select className="rounded-interactive border border-border bg-content px-3 py-2 text-[14px] outline-none transition focus:border-primary" value={values.status} onChange={(event) => updateValue('status', event.target.value as PackageStatus)}>
